...
public class FlywayScriptGeneratorRunner {
	...
	public static void main(String[] args) {
		...
		if (!validate(args)) {
			...
			if (args.length == 5) {
				...
				if (args[5].equalsIgnoreCase(SupportedDBTypes.POSTGRESQL.toString())) {
					dbSupport = SupportedDBTypes.POSTGRESQL.getDbSupport();
				} else if (args[5].equalsIgnoreCase(SupportedDBTypes.ORACLE.toString())) {
					dbSupport = SupportedDBTypes.ORACLE.getDbSupport();
				}
			}

			try {
				dataMigrator = new FlywayScriptGenerator(args[4], args[0]);
	...
	public static boolean validate(String[] args) {
		if (args.length < 5 || args.length > 6) {
			System.err.println("Usage: FlywayScriptGeneratorRunner [revision location] [start version] [end version] [filename] [prefix] [database_type]");
			...
			System.err.println("Prefix = prefix of revision files");
			...
			if (!args[5].equalsIgnoreCase(SupportedDBTypes.ORACLE.toString()) && !args[5].equalsIgnoreCase(SupportedDBTypes.POSTGRESQL.toString())) {
